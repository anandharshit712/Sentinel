...
**Document role:** Master solution specification. This document is the single source of truth for *what the system is and how it works*. The DFD ([02-dfd.md](02-dfd.md)), HLD ([03-hld.md](03-hld.md)), LLD ([04-lld.md](04-lld.md)) and Architecture Diagram ([05-architecture-diagram.md](05-architecture-diagram.md)) are all derived from this document and must not contradict it.
...
2. **Smart Test Selection** — the system reasons over *what actually changed* (diff + dependency graph + test-to-source mapping) and runs only the relevant subset of the project's existing test suite, plus an always-on smoke safety net.
3. **Explainable Promotion Gating** — review findings, test results, change profile and environment context converge into one deterministic risk score; a Promotion Gating Agent applies a graduated **trust ladder** and outputs *Promote / Hold / Escalate* with a complete, human-readable reasoning trail.

**The differentiator is cross-stage signal flow:** a Critical security finding raised during review directly raises the promotion risk score and can force human escalation *even when every test passes* — automatically, with the reasoning visible. Solving P4 is what makes P1–P3 solutions compound instead of remaining three disconnected tools.
...
| D5 | **Graduated trust ladder** | Full automation only where mistakes are cheap (dev→test). Mandatory human approval where they are expensive (staging→production — *always*, hard-coded, regardless of score). Thresholds are configuration, so automation can widen as trust builds. |
...
- Steps 3's two review agents and step 7 are **independent by contract** — the frontman issues those tool calls in parallel (LLM parallel tool-calling); the pipeline order for dependent stages is enforced by instruction and by the fact that each stage's *required input contract* only exists in `sly_data` after its producer ran.
...
Full HOCON, instructions text, and function schemas are in the LLD ([04-lld.md](04-lld.md) §3). This section defines each agent's *contractual behavior*.
...
|---|---|
| LLM | `nvidia-llama-3.3-70b-instruct` |
| CodedTools | `secret_scanner_tool` (regex+entropy secrets ruleset), `dependency_cve_tool` (OSV.dev lookup of manifest deltas, offline snapshot fallback) |
...
| CodedTools | `complexity_metrics_tool` (cyclomatic complexity + function length; radon for Python, heuristic parser for JS/TS) |
...
|---|---|
| LLM | `nvidia-llama-3.3-70b-instruct` |
| CodedTools | `incident_history_tool` (Postgres: incidents/reverts for repo+target env in lookback window), `deploy_window_tool` (freeze calendar & risky-window policy: e.g. Friday ≥16:00 local, weekends, release freezes) |
...
| `security_findings` | security_review_agent | review_synthesis |
| `quality_findings` | code_quality_agent | review_synthesis |
| `review_report` | review_synthesis_agent | risk_scoring, Gateway (upstream) |
| `test_plan` | test_selection_agent | test_runner_tool |
| `test_results` | test_runner_tool | risk_scoring, Gateway (upstream) |
| `env_context` | environment_context_agent | risk_scoring |
...
*Worked check (Demo Run 2):* 1 Critical SQLi finding (+40) + auth sensitive-area flag (+15) + all tests green (0) = **55 (high)** minimum — gating on `test→qa` or higher escalates, even though CI is green. Binary gating structurally cannot do this.
...
5. **review_synthesis_agent** → `review_report`, published to developer (dashboard + PR comment) *in seconds* — the review deliverable is done here even before tests run.
...
The Gateway implements one **adapter interface** (`inbound: webhook→DeliveryEvent`, `outbound: decision→platform action`, `report: review→PR/MR comment`) with three concrete adapters. Full endpoint/payload detail: LLD §7; pipeline snippets: LLD §7.4.
...
Served by the Gateway (REST + SSE + static single-page UI). Screens (wireframes + API mapping: LLD §9):
...
- LLM safety: reviewed code is untrusted input — review agents' instructions explicitly refuse in-code instructions; the gating path is deterministic (trust ladder tool), and the LLM can only *raise* risk (§6) — a prompt-injected diff cannot talk the system into auto-promoting.
...
1. ✅ Working Neuro-SAN network — all 10 components (§5.3) configured in `registries/delivery_intelligence.hocon`, 16 coded tools implemented.
...
4. ✅ **Demo Run 2 — the escalation (money shot):** change touching the auth module with planted string-concatenated SQL → Security Review flags **Critical** → risk ≥75 **although every test passes** → gating escalates to human approval, reasoning trail explicitly citing the security finding; approver resolves it live in the dashboard.
5. ✅ Side-by-side reasoning trails (dashboard) + live agent choreography (NSFlow).
6. Stretch: live GitHub webhook (not simulated); seeded `incidents` row visibly shifting a repeat run's score; Slack/Teams notification on the approval step.

**Non-goals (hackathon):** no new CI engine, no human-reviewer replacement, no test *generation*, no auto-deploy to production under any configuration, no multi-tenant SaaS hardening.
