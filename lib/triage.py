...
DEFAULT_EXCLUDE_DIRS: List[str] = [
    ...
    ".idea", ".gradle", "Pods", "coverage", ".next",
]
# Generated / binary-ish file names, matched against the basename (and the full path, so the
# `<Name>.xcodeproj/` forms work — those dirs are named per-project, not by a fixed segment).
DEFAULT_EXCLUDE_GLOBS: List[str] = [
    ...
    # IDE / build project files: machine-written, all-identifier content, zero review value.
    "*.pbxproj", "*.xcodeproj/*", "*.xcworkspace/*", "*.pbxuser", "*.xcuserstate", "*.iml",
]

# A generator banner in the first few added lines marks the whole file as machine-written.
# protobuf/openapi/prisma/graphql-codegen/swagger all stamp one, so this catches generated
# files no glob list has heard of. PR-mode caveat: only fires if the banner is inside a hunk —
# in audit mode the whole file is "added", so it always is.
GENERATED_MARKER = re.compile(r"(?i)@generated\b|do not edit|code generated by|auto-?generated")
...
# High-entropy literal heuristic, shared with secret_scanner so the deterministic floor and the
# hotspot ranking that feeds the LLM reviewer agree on what "looks like a secret" means.
_QUOTED = re.compile(r"['\"]([^'\"]{20,})['\"]")
# A real secret (base64/hex/JWT/token) is compact and structureless: token chars only, no spaces,
# no ':' — so URLs and prose are rejected before the entropy test.
TOKENISH = re.compile(r"^[A-Za-z0-9+/=_.\-]{20,}$")
# ...but a reverse-DNS identifier is token-shaped AND high-entropy (`android.permission.
# ACCESS_FINE_LOCATION` scores 4.37). Namespaced identifiers are never secrets — reject them.
DOTTED_IDENT = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*){2,}$")
...
def looks_like_secret(s: str) -> bool:
    """True if `s` is token-shaped, not a namespaced identifier, and high-entropy."""
    return bool(TOKENISH.match(s)) and not DOTTED_IDENT.match(s) and entropy(s) >= ENTROPY_THRESHOLD
...
def score_line(path: str, content: str, sensitive_files: set) -> Tuple[float, List[str]]:
    ...
    if m and looks_like_secret(m.group(1).strip()):
        score += 1.5
...
def generated_files(items: List[Tuple[str, int, str]], head: int = 8) -> set:
    """Paths carrying a generator banner within their first `head` added lines."""
    seen: Dict[str, int] = {}
    gen = set()
    for path, _lineno, content in items:
        n = seen.get(path, 0)
        if n >= head:
            continue
        seen[path] = n + 1
...
def scored_lines(lines: Iterator[Tuple[str, int, str]], sensitive_files: set,
    ...
    items = [t for t in lines if not is_excluded(t[0], extra_globs)]
    generated = generated_files(items)
    out: List[Dict[str, Any]] = []
    for path, lineno, content in items:
...
def demo() -> None:
    ...
    assert is_excluded("App/ios/App.xcodeproj/project.pbxproj")
    assert is_excluded("android/app/App.iml")

    # entropy gate: namespaced identifiers are token-shaped AND high-entropy but never secrets
    assert not looks_like_secret("android.permission.ACCESS_FINE_LOCATION")  # 4.37, dotted ident
    assert not looks_like_secret("https://example.com/a/very/long/path")     # not token-shaped
    assert looks_like_secret("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY")     # real AWS secret key

    # a generator banner drops the whole file from the reviewable surface
    gen_profile = {"files": [
        {"path": "api/schema_pb2.py", "hunks": [{"new_start": 1, "patch":
            "@@\n+# Code generated by protoc. DO NOT EDIT.\n+TOKEN = 'aGVsbG8gd29ybGQgc2VjcmV0Cg'\n"}]},
        {"path": "api/handler.py", "hunks": [{"new_start": 1, "patch": "@@\n+x = compute()\n"}]}]}
    paths = {d["file"] for d in scored_lines(iter_added_lines(gen_profile), set())}
    assert paths == {"api/handler.py"}, paths
