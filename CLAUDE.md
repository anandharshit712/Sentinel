...
**Done, not yet committed (2026-07-08):** **0.3** `samples/python-payments-service` (Flask+pytest, auth module = Demo-2 SQLi plant site, real smoke test; 6 tests). **A1** coded tools `git_diff` + `ast_analyzer` + `dependency_graph` — **stdlib `ast`+`git`, zero new deps** (dodges tree-sitter/radon Windows friction; JS/TS deferred with node repo). Cooperate via `sly_data["change_profile_wip"]`; `dependency_graph` finalizes → validated `change_profile`. 4 golden-diff tests (modified/added/deleted/renamed/binary). Added `pytest.ini` scoping to `tests/` (samples + studio run separately). Flask installed in `.venv` for the sample suite. Main suite 16 passing.

**Next (critical path, [07](docs/solution/07-implementation-plan.md)):**
1. Track A rest: **A2** (`secret_scanner`, `dependency_cve`), **A3** (`complexity_metrics`), **A7** (`contract_store`, `report_publisher`, `decision_logger`, `cicd_action`, `notification`) — recommended order feeds B2.
